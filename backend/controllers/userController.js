...
export const verify = async (req, res) => {
  try {
    ...
    const token = authHeader.split(" ")[1]; //[Bearer, ghjfjhdjsasdsad]
    let decoded;
    try {
...
export const updateUser = async (req, res) => {
  try {
    ...
    // ✅ SEND SAFE USER (NO PASSWORD)
    const safeUser = await User.findById(userIdToUpdate).select("-password");
    ...
    console.log("UPDATE USER ERROR =>", error); // 👈 VERY IMPORTANT FOR DEBUG
    return res.status(500).json({
      success: false,
      message: error.message,
    });
